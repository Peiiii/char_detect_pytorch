...
class Renderer:
    def __init__(self,font_dir=cfg.ROOT+'/data/fonts',bg_dir=cfg.ROOT+'/data/bgs',bg_mode='RGBA',scale=1.5,charset=None):
        ...
        # self.canvas_color=(0,0,0,100)
        # self.out_size=(int(self.font_size*1.5),int(self.font_size*1.5))
        self.out_size=(int(self.font_size*scale),int(self.font_size*scale))
        # self.out_size=(int(self.font_size),int(self.font_size))
    ...
    def get_random_text_color(self):
        ...
        if n<0.2:
            ...
            color=random.randint(200,256),random.randint(200,256),random.randint(200,256),random.randint(200,256)
        elif n<0.6:
            color=random.randint(200,256),random.randint(200,256),random.randint(200,256),random.randint(150,256)
        elif n<0.7:
            a=170
    ...
    def paste_canvas_on_bg(self,canvas,bg):
        ...
        return bg
    def apply_rotate_on_canvas(self,img,max_angle=45,fillcolor=(0,0,0,100)):
        n=random.random()
        if n<0.06:
            angle=random.randint(-180,180)
        elif n<0.55:
            angle=utils.get_randn_clipped([0,90])
        else:
            angle=utils.get_randn_clipped([-90,0])
    ...
    def apply_rot_on_canvas(self,img,angel=20, max_angel=80):
        ...
        img = self.rot(img, angel=angel, shape=self.out_size, max_angel=max_angel)
        return img
    ...
    def gen_one_img(self,char):
        ...
        canvas=GD.distort_img(self.pil2np(canvas))
    ...
    def rot(self,img, angel, shape, max_angel):
        """
            添加放射畸变
            img 输入图像
            factor 畸变的参数
